...
struct VoicePackCenterCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Toggle("启用自然语音", isOn: $settings.useNaturalVoice)
                .tint(Theme.vegetable)
                .font(.system(size: Theme.scaled(15, settings: settings), weight: .semibold, design: .rounded))
                .foregroundStyle(Theme.ink)

            Text(settings.useNaturalVoice
                 ? "已启用：下载语音包后，朗读自动改用开源模型发声。"
                 : "已关闭：使用系统语音朗读（第一版方案），无需下载，开箱即用。")
                .font(.system(size: 13))
                .foregroundStyle(Theme.inkSoft.opacity(0.85))
                .lineSpacing(3)

            Text("开源 TTS 模型（sherpa-onnx 离线推理），发音比系统语音更自然。普通话和英语系统语音可用但较机械；粤语系统内置语音（Sinji）可用，想要更自然的粤语推荐下载。")
                .font(.system(size: 13))
                .foregroundStyle(Theme.inkSoft.opacity(0.85))
                .lineSpacing(3)
            ...
            Text("下载需要联网，建议在 Wi-Fi 下进行；中断后可点「继续」从断点续传。")
                .font(.system(size: 12))
...
extension VoicePackManager.State {
    var buttonTitle: String {
        if case .failed = self { return "继续" }   // 从断点续传，不重头下
        return "下载"
    }
}
